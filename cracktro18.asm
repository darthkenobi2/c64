// Status:
// I commented out the loading of the charset as that wouldn't asselble
// due to overlap

// I don't understand the math routines  what they are good foe
// If you are calculating the start of character rows, then do that 
// into a table and your can cut lots of code and save CPU time
...
// There is an overlap here at $6000 so KickAssembler will complain
// I norally do .import c64 "picture",$0,$1f40 to read thhe bitmap part of
// the picture. Your variant is fancier but mine works as well.
...
// Here a general copy loop would do the job. Your routine
// generates a massively long row of LDA/STA
// I am aware I copy the same values several times
// but given that you don't need a cpx #$ on every lap,
...
//    dex
//    bpl !-
// or make a full loop with all the values of VIC you want to copy
...
// Here I would do .var screen1 = $4400 and .var screen2 = $4c00 in the beginning
// and then use lda #>screen1 and lda #>screen2
// That makes it possible to move screens by justy moving the definitions
  ...
  // to save bytes lsr $d019 is what the cool guys do .-)

	cli

loopy:
	jsr scroller  	// Scroller runs outside the interrupt?
        .for (var l=0; l<750; l++) {
           nop
        }
...
multiply:
	lda #$02
	cmp #$00  		//CMP with zero not needed. the Z flag is set by the LDA of zero
	beq !+++++++++   // That is worth a label and keeping track of the ! is surely error prone ;-)
	dec $02
	lda $02
	sta $25
	lda #$00
...
shift_screen:
	...
	lda #$38
	sta $d018
	jmp !++ 	//Cool kids would BNE - that saves a byte :-)
!:	lda #$18
	sta $d018
!: 	rts
...
//	bne !+ //if not @
	beq zero
// Here we already know it's zero, don't we?
//	ldx #00 //if it is @
//	lda scrolltext,x
//	clc
//	adc #63
//	jmp !+++  	//You have the adc below - why not jump directly there?
!: 	cmp #32 //is it space
	bne !+ //not space
...
!: 	sta $47f8+setup  // As above screen1+$03f8
	sta $4ff8+setup  // As above screen2+$03f8
	inx
	}
	dex
	rts            
...
!: 	sta $47f8+pickle    //screen1+$03f8+pickle
	sta $4ff8+pickle 	//screen2+$03f8+pickle
!: 	dec $d000+pickle*2
	}
	rts
...
!:	lda $d012
	...
	// Here inside the IRQ keep the stores together 
	// It still flickers a bit, but should be less
	...
	// you need to store d018 here as well. That is set inside the 
	// shift_screen routine. 
	// I think that is why you have the flicker
